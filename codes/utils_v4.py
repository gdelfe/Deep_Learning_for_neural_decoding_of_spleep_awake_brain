...
class SpectrogramDatasetNew(Dataset):
    def __init__(self, files, load_path, CH=None, lower_band = 0, higher_band = 100):
        self.CH = CH
        self.files = files
        self.load_path = load_path
        self.lower_band = lower_band
        self.higher_band = higher_band
    ...
    def __getitem__(self, idx):
        ...
        if self.CH is not None:
            ...
            return torch.transpose(spec[:,:,self.lower_band:self.higher_band+1], 2, 1), torch.Tensor([label]), date, rec, time
...
def create_dataloaders(train_files, val_files, test_files, load_path, batch_size=32, CH=None, lower_band = 0, higher_band = 100):
    train_dataset = SpectrogramDatasetNew(files=train_files, load_path=load_path, CH=CH, lower_band = lower_band, higher_band = higher_band)
    valid_dataset = SpectrogramDatasetNew(files=val_files, load_path=load_path, CH=CH, lower_band = lower_band, higher_band = higher_band)
    test_dataset = SpectrogramDatasetNew(files=test_files, load_path=load_path, CH=CH, lower_band = lower_band, higher_band = higher_band)
...
def get_loss(model, labels, outputs, alpha=0, timewindow=10, loss_type='bce', reg_type='none', reduction='mean', freq = 100):
    if loss_type == 'hinge':
        labels[labels == 0] = -1
        if reduction == 'mean':
    ...
    if reg_type != 'none':
        weights = model.linear.weight.view(-1, freq, timewindow)
    if reg_type == 'l2':
...
def train(model, optimizer, loader, alpha, timewindow=10, model_type='LR', loss_type='bce', reg_type=None, collect_result=False, device='cuda', freq = 100):
    model.train()
    batch_losses = 0
    batch_lengths = 0
    ...
    for batch_idx, (data, labels, _, _, _) in enumerate(loader):
        ...
        loss = get_loss(model, labels, outputs, alpha=alpha, timewindow=timewindow, loss_type=loss_type, reg_type=reg_type,reduction='sum', freq = freq)
        batch_losses += loss
...
def evaluate(model, optimizer, loader, alpha, timewindow=10, model_type='LR', loss_type='bce', reg_type=None, collect_result=False, device='cuda', freq = 100):
    model.eval()
    batch_losses = 0
    batch_lengths = 0
...
def plot_weight_glm(device, path, model_type, CH, loss_type, reg_type, alpha, best_epoch, timewindow=10, freq = 100):
    if CH == 'all':
        model = GLM(input_dim=freq*timewindow*62).to(device)
        ...
        weights = model.linear.weight.view(62, freq, timewindow)
        ...
        model.load_state_dict(torch.load('{}/{}_CH{}_LOSS{}_REG{}{}_TW{}_EPOCH{}.pt'.format(path, model_type, CH, loss_type, reg_type, alpha, timewindow, best_epoch)))
        weights = model.linear.weight.view(freq, timewindow)
...
def tuning(train_loader, val_loader, model, optimizer, device, num_epochs, alpha, model_type, loss_type, reg_type, CH, path, timewindow=10, verbose=False, freq = 100):
    training_losses, training_acc, val_losses, validation_acc = [], [], [], []
    for epoch in range(num_epochs):
        train_loss, train_acc = train(model, optimizer, train_loader, alpha=alpha, timewindow=timewindow, model_type=model_type, loss_type=loss_type, reg_type=reg_type, collect_result=False, device=device, freq = freq)
        val_loss, val_acc = evaluate(model, optimizer, val_loader, alpha=alpha, timewindow=timewindow, model_type=model_type, loss_type=loss_type, reg_type=reg_type, collect_result=False, device=device, freq = freq)
