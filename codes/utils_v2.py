...
class SpectrogramDataset(Dataset):
    def __init__(self, val_dates, test_dates, mode='train', version='_Goose_1st', CH=None, upsample=False):
        ...
        self.movement_files = os.listdir('/home/bijanadmin/Desktop/Goose_data/'+version+'/move/')
        self.sleeping_files = os.listdir('/home/bijanadmin/Desktop/Goose_data/'+version+'/sleep/')
        if upsample:
    ...
    def __getitem__(self, idx):
        ...
        path = '/home/bijanadmin/Desktop/Goose_data/'
        spec = torch.from_numpy(np.load(path+self.version+'/'+ mvmt_type +'/' +self.all_files[idx])) 
        if mvmt_type == 'move':
...
#     def __init__(self, version='_Goose_1st_TVT', CH=None):
#         self.CH = CH
#         self.version = version
#         movement_files = os.listdir('/mnt/pesaranlab/People/Capstone_students/Yue/data/data'+version+'/move/train/')
#         sleep_files = os.listdir('/mnt/pesaranlab/People/Capstone_students/Yue/data/data'+version+'/sleep/train/')
        
#         # diff = len(self.sleep_files)-len(self.movement_files)
#         # try:
#         #     d = 0
#         #      while d < diff:
#         #             ind = random.randint(0, len(self.movement_files)-1)
...
#         movement_labels = [0]*len(movement_files)
#         sleep_labels = [1]*len(sleep_files)
        
#         self.movement = [(movement_files[i], movement_labels[i]) for i in range(0,len(movement_files))]
#         self.sleep = [(sleep_files[i], sleep_labels[i]) for i in range(0,len(sleep_files))]
        
#         self.data = self.movement + self.sleep 
  
#     def __len__(self):
#         return len(self.data)
    
#     def __getitem__(self, idx):
#         class_type  = self.data[idx][0].split('_')[-1].split('.')[0]
#         date = self.data[idx][0].split('_')[0]
#         rec = self.data[idx][0].split('_')[1].split('_')[0]
#         time = float(self.data[idx][0].split('_')[3][4:])
#         path = '/mnt/pesaranlab/People/Capstone_students/Yue/data/'
#         spec = torch.from_numpy(np.load(path+'data'+ self.version+'/'+ class_type +'/train/' +self.data[idx][0])) 
...
# def create_dataloaders(version='v5', batch_size=32, CH=None, upsample=False):

#     train_dataset = Dataset_train(version=version, CH=CH)
#     train_loader = DataLoader(dataset = train_dataset, batch_size = batch_size, shuffle = True)
...
def train(model, criterion, optimizer, loader, alpha, model_type='LR', loss_type='bce', reg_type=None, collect_result=False, device='cuda'):
    model.train()
    batch_losses = 0
    batch_lengths = 0
