...
save_path = '/mnt/pesaranlab/People/Capstone_students/Yue/datav5/'


window = 10

# nights = ['180326', '180327', '180328', '180329', '180330', '180331', '180401', '180409', '180410', '180411', '180412', '180413', '180414']
    ...
    for rec in recs:
        ...
        try: 
            ...
            for state in movement_states:
                start = state[0][0]
                start_idx =  (np.abs(times-start)).argmin() # include windows before actual movement?
                stop = state[0][1]
            ...
            #TODO: For camera fall, out of frames. # Is it still a todo now?
            ...
            for s in sleep_states_idx:
                ...
                stimes = np.split(stime[:-(sspecs[-1].shape[1])], len(sspecs)-1)
                stimes_mid = stimes[:, window//2]

                for i, arr in enumerate(sspecs):
                    if not torch.isnan(arr.sum()) and arr.shape[1] == window:
                        np.save(save_path + 'sleep/' + night + '_' + rec + '_' + str(ch) + '_win'+str(i) +'_time'+str(stimes_mid[i])+'_sleep.npy',arr.numpy())
            ...
            for s in movement_states_idx:
                ...
                for i, arr in enumerate(sspecs):
                    if not torch.isnan(arr.sum()) and arr.shape[1] == window:
                        np.save(save_path + 'move/' + night + '_' + rec + '_' + str(ch) + '_win'+str(i) +'_time'+str(stimes_mid[i])+'_move.npy',arr.numpy())
